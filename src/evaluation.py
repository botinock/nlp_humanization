...
def compute_rouge_batch_parallel(predictions, references, num_workers=4):
    """Computes ROUGE scores in parallel using multiprocessing."""
    chunk_size = len(predictions) // num_workers
    chunks = [
        (predictions[i: i + chunk_size], references[i: i + chunk_size])
        for i in range(0, len(predictions), chunk_size)
    ]

    with Pool(processes=num_workers) as pool:
        rouge_results = pool.starmap(rouge_evaluator.compute, chunks)

    # Merge results
    final_scores = {key: sum(d[key] for d in rouge_results) / len(rouge_results) for key in rouge_results[0]}
    return final_scores
...
# def evaluate_model(model, eval_loader, tokenizer, device, epoch, writer, num_workers=4):
#     """Evaluates the model using ROUGE, BLEU, Cosine Similarity, BERTScore, and Evaluation Loss."""
#     model.eval()
...
#     with torch.no_grad():
#         for batch in tqdm(eval_loader, desc=f"Evaluating Epoch {epoch}", leave=False):
...
#             # Collect for ROUGE & BERTScore
#             all_references.extend(human_texts)
#             all_candidates.extend(generated_texts)

#             bleu_scores = [compute_bleu(ref, pred) for ref, pred in zip(human_texts, generated_texts)]
#             cosine_scores = [compute_cosine_similarity(pred, ref) for pred, ref in zip(generated_texts, human_texts)]

#             total_bleu += sum(bleu_scores)
#             total_cosine += sum(cosine_scores)

#     # Compute ROUGE in one batch
#     rouge_scores = compute_rouge_batch_parallel(all_candidates, all_references)
...
#     return avg_loss, rouge_scores, avg_bleu, avg_cosine, avg_bertscore
